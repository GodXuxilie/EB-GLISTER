...
class DataSelectionStrategy(object):
    ...
    def __init__(self, trainloader, valloader, model, num_classes, linear_layer, loss, device, logger, emb_weight=0, length=36, emb_dim=300):
        """
        Constructor method
        """
    ...
    def compute_gradients(self, valid=False, perBatch=False, perClass=False):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if batch_idx == 0:
                ...
                # print(batch_idx, l0_grads.shape, emb_grads.shape)
                # print(self.model.embedding.weight.grad.shape)
            else:
                out, l1, emb_out = self.model(inputs, last=True, freeze=False, emb_last=True)
                loss = self.loss(out, targets).sum()
                ...
                # print(batch_idx, batch_l0_grads.shape, batch_emb_grads.shape)
        ...
        if self.emb_weight > 0:
            self.emb_grads_per_elem = emb_grads
            # print(self.emb_grads_per_elem.shape)
        ...
        if valid:
            ...
            if self.emb_weight > 0:
                self.val_emb_grads_per_elem = emb_grads
