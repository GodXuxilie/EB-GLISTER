...
class GLISTERStrategy(DataSelectionStrategy):
    ...
    def __init__(self, trainloader, valloader, model, 
                ...
                logger, r=15, emb_weight=0, emb_dim=300, length=36):
        """
        Constructor method
        """
        super().__init__(trainloader, valloader, model, num_classes, linear_layer, loss_func, device, logger, emb_weight, emb_dim, length)
    ...
    def _update_grads_val(self, grads_curr=None, first_init=False, emb_grads_curr=None):
        """
        ...
        if first_init:
            for batch_idx, (inputs, targets) in enumerate(valloader):
                ...
                # exit(0) 
                if batch_idx == 0:
                    out, l1, emb_out = self.model(inputs, last=True, freeze=False, emb_last=True)
                    loss = self.loss(out, targets).sum()
                    l0_grads = torch.autograd.grad(loss, [out, emb_out])
                    emb_grads = l0_grads[1].cpu()
                    l0_grads = l0_grads[0].cpu()
                    if self.linear_layer:
                        l0_expand = torch.repeat_interleave(l0_grads, embDim, dim=1)
                        l1_grads = l0_expand * l1.repeat(1, self.num_classes)
                    self.init_out = out
                    self.init_emb_out = emb_out.cpu()
                    self.init_l1 = l1
                    ...
                    out, l1, emb_out = self.model(inputs, last=True, freeze=False, emb_last=True)
                    loss = self.loss(out, targets).sum()
                    batch_l0_grads = torch.autograd.grad(loss, [out, emb_out])
                    batch_emb_grads = batch_l0_grads[1].cpu()
                    batch_l0_grads = batch_l0_grads[0].cpu()
                    if self.linear_layer:
                    ...
                    emb_grads = torch.cat((emb_grads, batch_emb_grads), dim=0)
                    if self.linear_layer:
                        l1_grads = torch.cat((l1_grads, batch_l1_grads), dim=0)
                    self.init_out = torch.cat((self.init_out, out), dim=0)
                    self.init_emb_out = torch.cat((self.init_emb_out, emb_out.cpu()), dim=0)
            ...
            # print(self.init_out.shape ,grads_curr.shape, self.init_emb_out.shape, emb_grads_curr.shape)

            out_vec = self.init_out - (
                    self.eta * grads_curr[0][0:self.num_classes].view(1, -1).expand(self.init_out.shape[0], -1)).to(self.device)
            ...
            if self.emb_weight > 0:
                emb_out_vec = self.init_emb_out.to(self.device) - (
                    self.eta * emb_grads_curr[0].to(self.device).view(1, self.length, self.emb_dim).expand(self.init_emb_out.shape[0], -1, -1))

            if self.emb_weight > 0:
                out, l1, emb_out = self.model(emb_out_vec, last=True, freeze=False, emb_last=True, emb_init=True)
                loss = self.loss(out, self.y_val.view(-1)).sum()
                l0_grads = torch.autograd.grad(loss, [out, emb_out])
                emb_grads = l0_grads[1].cpu()
                l0_grads = l0_grads[0].cpu()
            else:
                loss = self.loss(out_vec, self.y_val.view(-1)).sum()
                l0_grads = torch.autograd.grad(loss, [out_vec])[0].cpu()
        ...
        if self.emb_weight > 0:
            self.emb_grads_val_curr = torch.mean(torch.mean(emb_grads, dim=1).permute(1,0), dim=1).view(-1, 1)
            # print(self.emb_grads_val_curr.shape)

    def eval_taylor_modular(self, grads, emb_grads=None):
        """
        Evaluate gradients
        ...
        grads_val = self.grads_val_curr.to(self.device)
        if self.emb_weight > 0:
            emb_grads_val = self.emb_grads_val_curr.to(self.device)
        with torch.no_grad():
            # print(grads.shape, grads_val.shape)
            gains = torch.matmul(grads.to(self.device), grads_val)
            if self.emb_weight > 0:
                # print(torch.mean(emb_grads, dim=1).shape)
                gains += self.emb_weight * torch.matmul(torch.mean(emb_grads.to(self.device), dim=1), emb_grads_val)
        # print(gains.shape)
        return gains

    def _update_gradients_subset(self, grads, element, emb_grds_curr=None):
        """
        ...
        if emb_grds_curr is not None:
            emb_grds_curr += self.emb_grads_per_elem[element].sum(dim=0).view(1, self.length, self.emb_dim)
    ...
    def greedy_algo(self, budget):
        ...
        if self.greedy == 'RGreedy':
            ...
            while (numSelected < budget):
                ...
                emb_rem_grads = self.emb_grads_per_elem[subset_selected]
                gains = self.eval_taylor_modular(rem_grads, emb_grads=emb_rem_grads)
                ...
                if numSelected == 0:
                    ...
                    if self.emb_weight > 0:
                        emb_grads_curr = self.emb_grads_per_elem[selected_indices].sum(dim=0).view(1, self.length, self.emb_dim)
                ...
                if self.emb_weight > 0:
                    self._update_grads_val(grads_curr, emb_grads_curr=emb_grads_curr)
                else:
        ...
        # Stochastic Greedy Selection Algorithm
            ...
            while (numSelected < budget):
                ...
                if numSelected > 1:
                    if self.emb_weight > 0: 
                        self._update_gradients_subset(grads_curr, bestId, emb_grads_curr)
                    else:
                    ...
                    if self.emb_weight > 0:
                        emb_grads_curr = self.emb_grads_per_elem[bestId].sum(dim=0).view(1, self.length, self.emb_dim)
                # Update the grads_val_current using current greedySet grads
                if self.emb_weight > 0:
                    self._update_grads_val(grads_curr, emb_grads_curr=emb_grads_curr)
                else:
            ...
            while (numSelected < budget):
                ...
                if numSelected == 1:
                    ...
                    if self.emb_weight > 0:
                        emb_grads_curr = self.emb_grads_per_elem[bestId[0]].view(1, self.length, self.emb_dim)
                else:  # If 1st selection, then just set it to bestId grads
                    if self.emb_weight > 0: 
                        self._update_gradients_subset(grads_curr, bestId, emb_grads_curr)
                    else:
                ...
                # Update the grads_val_current using current greedySet grads
                if self.emb_weight > 0:
                    self._update_grads_val(grads_curr, emb_grads_curr)
                else:
                    self._update_grads_val(grads_curr)
