...
class SSTDataset(Dataset):
    ...
    def __init__(self, path_to_dataset, name, num_classes, wordvec_dim, wordvec, device='cpu'):
        ...
        self.phrase_vec[0] = torch.nn.ConstantPad1d((0, max_len - self.phrase_vec[0].shape[0]), 0)(self.phrase_vec[0])
        # pad all seqs to desired length
        self.phrase_vec = pad_sequence(self.phrase_vec, batch_first=True)
        self.phrase_vec = self.phrase_vec[:,:36]

        # self.phrase_vec = torch.nn.utils.rnn.pad_sequence(self.phrase_vec)
        print(self.phrase_vec.shape)
